...
export function Instagram({ badge, headline, handle, href, posts }: InstagramProps) {
  const track = [...posts, ...posts];

  return (
    <section id="instagram" className="py-30 flex flex-col gap-[50px]">
      {/* Heading */}
      ...
      <div className="overflow-hidden">
        ...
        >
          {track.map((post, i) => (
            <InstagramCard key={`${post.id}-${i}`} post={post} />
          ))}
        </div>
      </div>
    </section>
